...
class Post_Plugin
{
    function render()
    {
        ...
        <script src="<?php echo Freewind_Helper::freeCdn('plugin/fw-editor/fw.editor.min.js') ?>"></script>
        <script>
            $(function () {
                if (window['postwrite']) return;
                window['postwrite'] = true;
                layui.use('colorpicker', function () {
                    $('body').prepend(`<div id="freewind-mask">
            <div class="freewind-file-box">
                <h2>附件设置 <span><i class="fa fa-close" id="close-file">  </i></span></h2>
                <div class="freewind-file-item">
                    <label for="postFile">附件类型</label>
                    <select id="postFile">
                        <option value="1" selected>关闭</option>
                        <option value="2">开启</option>
                        <option value="3">开启登录可见</option>
                        <option value="4">开启回复可见</option>
                        <option value="5">开启登录回复可见</option>
                    </select>
                </div>
                <div class="freewind-file-item">
                    <label for="postFileName">附件名称</label>
                    <input type="text" id="postFileName">
                    <label for="postFileSize">附件大小</label>
                    <input type="text" id="postFileSize">
                </div>
                <div class="freewind-file-item freewind-item-block">
                    <label for="postFileGuan">默认下载</label>
                    <input type="text" id="postFileGuan">
                </div>
                <div class="freewind-file-item freewind-item-block">
                    <label for="postFileBaidu">百度网盘</label>
                    <input type="text" id="postFileBaidu">
                </div>
                <div class="freewind-file-item freewind-item-block">
                    <label for="postFileLan">蓝奏云网盘</label>
                    <input type="text" id="postFileLan">
                </div>
                <div class="freewind-file-item freewind-item-block">
                    <label for="postFile360">360网盘地址</label>
                    <input type="text" id="postFile360">
                </div>
                <div class="freewind-file-item freewind-item-block">
                    <label for="postFileCheng">城通网盘地址</label>
                    <input type="text" id="postFileCheng">
                </div>
                <div class="freewind-file-item freewind-item-block">
                    <label for="postFileTian">天翼网盘地址</label>
                    <input type="text" id="postFileTian">
                </div>
                <div class="freewind-file-item freewind-item-block">
                    <label for="postFileWei">微云网盘地址</label>
                    <input type="text" id="postFileWei">
                </div>
                <div class="freewind-file-item freewind-item-block">
                    <label for="postFileGit">Github地址</label>
                    <input type="text" id="postFileGit">
                </div>
                <p style="color: #999999;padding-left: 100px;">
                    若有提取码，则直接在下载地址后面拼上||提取码<br>如:<code>https://pan.baidu.com/s/1qJN6wxHkWj-G_rdLB0pQOg||mlpk</code>
                </p>
                <div class="freewind-footer">
                    <button id="file-cancel" type="button">取消</button>
                    <button id="file-right" type="button">确认</button>
                </div>
            </div>
        </div>`)
                    let init = false;
                    let closeFile = function () {
                        $("#freewind-mask").fadeOut()
                    }
                    $("#close-file").on('click', closeFile)
                    $("#file-cancel").on('click', closeFile)
                    $("#file-right").on('click', function () {
                        let fileItem = $("#freewind-mask input,#freewind-mask select")
                        let fileInfo = {}
                        for (let i = 0; i < fileItem.length; i++) {
                            let item = $(fileItem[i])
                            fileInfo[item.attr('id')] = item.val()
                        }
                        console.log(fileInfo);
                        $("input[name=fields\\[postFile\\]]").val(JSON.stringify(fileInfo))
                        closeFile()
                    })
                    let fileStr = $("input[name=fields\\[postFile\\]]").val()
                    if (fileStr) {
                        let fileInfo = JSON.parse(fileStr)
                        console.log(fileInfo);
                        let keys = Object.keys(fileInfo)
                        for (let i = 0; i < keys.length; i++) {
                            $(`#${keys[i]}`).val(fileInfo[keys[i]])
                        }
                    }
                    ...
                    postType[3] = ["undo", "redo", "fullscreen", "help",]
                    let postTypeList = []
                    postTypeList[1] = ['fields\\[postShow\\]']
                    postTypeList[2] = ['fields\\[postShuoPic\\]', 'fields\\[postShuoType\\]']
                    let postShowList = []
                    postShowList[3] = ['fields\\[postShowImg\\]']
                    let postShuoList = []
                    postShuoList[1] = ['fields\\[postShuoPic\\]']
                    postShuoList[2] = ['fields\\[postShuoMusic\\]', 'fields\\[postShuoMP\\]', 'fields\\[postShuoMT\\]']
                    postShuoList[3] = ['fields\\[postShuoBvid\\]', 'fields\\[postShuoPage\\]']
                    $("#custom-field input[type=hidden]").parent().parent().parent().hide()
                ...
                <div class="fw-form-item"><label>提示内容：</label><textarea style="outline:none;width: 235px;resize: none;height: 100px;" id="fw-msg-content">${editor.cm.getSelections()['0']}</textarea></div>
                </div>
                `
                                    let idx = layer.open({
                                        type: 1,
                                        ...
                                        content: content,
                                        btn1: () => {
                                            editor.cm.replaceSelection(`{fwcode type="${$("#fw-msg-type").val()}"}\n${$("#fw-msg-content").val()}\n{/fwcode}`);
                                            editor.cm.focus()
                                            layer.close(idx)
                                        }
                                    })
                ...
                `
                                    let idx = layer.open({
                                        ...
                                        btn1: () => {
                                            ...
                                            let btncontent = `{fwbili bvid="${$("#fw-bvid").val()}" bvnu="${bvnu}"}{/fwbili}`
                                            editor.cm.replaceSelection(btncontent);
                                            editor.cm.focus()
                                            layer.close(idx)
                                        }
                ...
                `
                    ...
                    $("select[name=fields\\[postType\\]]").on('change', function () {
                        let type = parseInt($(this).val())
                        if (type > 3 || type < 0) {
                            editor.settings['menu'] = postType[1]
                        } else {
                            editor.settings['menu'] = postType[type]
                        }
                        editor.reoladToolBar();
                        for (let i = 1; i < postTypeList.length; i++) {
                            if (i === type) {
                                if (postTypeList[i]) {
                                    for (let j = 0; j < postTypeList[i].length; j++) {
                                        $(`#custom-field [name=${postTypeList[i][j]}]`).parent().parent().parent().show();
                                    }
                                }
                            } else {
                                if (postTypeList[i]) {
                                    for (let j = 0; j < postTypeList[i].length; j++) {
                                        $(`#custom-field [name=${postTypeList[i][j]}]`).parent().parent().parent().hide();
                                    }
                                }
                            }
                        }
                        if (type === 1) {
                            $("select[name=fields\\[postShuoType\\]]").val(0).change()
                            if (init) {
                                $("select[name=fields\\[postShow\\]]").val(1).change()
                            }
                        } else if (type === 2) {
                            if (init) {
                                $("select[name=fields\\[postShuoType\\]]").val(1).change()
                            }
                            $("select[name=fields\\[postShow\\]]").val(0).change()
                        } else if (type === 3) {
                            $("select[name=fields\\[postShuoType\\]]").val(0).change()
                            $("select[name=fields\\[postShow\\]]").val(0).change()
                        }
                    ...
                    $("select[name=fields\\[postShow\\]]").on('change', function () {
                        let type = parseInt($(this).val())
                        for (let i = 1; i < postShowList.length; i++) {
                            if (i === type) {
                                if (postShowList[i]) {
                                    for (let j = 0; j < postShowList[i].length; j++) {
                                        $(`#custom-field [name=${postShowList[i][j]}]`).parent().parent().parent().show();
                                    }
                                }
                            } else {
                                if (postShowList[i]) {
                                    for (let j = 0; j < postShowList[i].length; j++) {
                                        $(`#custom-field [name=${postShowList[i][j]}]`).parent().parent().parent().hide();
                                    }
                                }
                            }
                        }
                    }).change()
                    $("select[name=fields\\[postShuoType\\]]").on('change', function () {
                        let type = parseInt($(this).val())
                        for (let i = 1; i < postShuoList.length; i++) {
                            if (i === type) {
                                if (postShuoList[i]) {
                                    for (let j = 0; j < postShuoList[i].length; j++) {
                                        $(`#custom-field [name=${postShuoList[i][j]}]`).parent().parent().parent().show();
                                    }
                                }
                            } else {
                                if (postShuoList[i]) {
                                    for (let j = 0; j < postShuoList[i].length; j++) {
                                        $(`#custom-field [name=${postShuoList[i][j]}]`).parent().parent().parent().hide();
                                    }
                                }
                            }
                        }
